...
<template>
  <div class="container">
    <div class="block">
      <h2>区域1</h2>
      <p>
        <button @click="modalVisble = true">打开朦层</button>
      </p>
      <Teleport to="body">
        <Modal v-if="modalVisble">
          <button @click="modalVisble = false">关闭朦层</button>
        </Modal>
      </Teleport>
    </div>
    <div class="block mid">
...
<script>
import { getAsyncComponent } from "../Util/index";
import { ref } from "vue";
...
export default {
  ...
  setup() {
    const modalVisbleRef = ref(false);
    return {
      modalVisble: modalVisbleRef,
    };
  },
};
</script>
